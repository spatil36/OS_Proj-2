...
int sys_set_sched(void){
  //extern int pro_scheduler;

  if(!argint(0,&pro_scheduler)){
    if(pro_scheduler)
    myproc()->pro_prty=1;
  }
  return 0;
...
// Implementation for set_priorits system call function

int sys_set_priority(void){
  int pro_id,proc_prty;

  if(argint(0,&pro_id) < 0)
  return -1;
  if(argint(1, &proc_prty) < 0)
  return -1;
  set_priority(pro_id, proc_prty);
return 0;
